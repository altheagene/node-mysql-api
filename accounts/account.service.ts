...
async function authenticate({email, password, ipAdress} : any){
    ...
    const refreshToken = generateRefreshToken(account, ipAdress);
...
async function revokeToken({token, ipAdress} : any){
    const refreshToken = await getRefreshToken(token);
...
async function sendVerificationEmail(account: any, origin: any){
    ...
    if(origin){
        ...
        message = `<p>Please use the token below to verify your email address with the <code>/account/verofy-email</code> api route: </p>
...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
    ...
    await sendEmail({
        ...
        html: `<h4>Email Already Resgitered</h4>
                <p>Your email <strong>${email}</strong> is alrady registered.</p>
                ${message}`
    });
}
...
async function sendPasswordResetEmail(account: any, origin: any) {
    ...
    if (origin) {
        const resetUrl = `${origin}/account/reset-password?token=${account.resetPassword}`;
